    ...
    try {
          ...
          if (qty == null) {
            fetch("/api/cart", { credentials: "same-origin" })
              ...
              .then((json) => {
                if (!json) return;
                const items =
                  json.items || (json.cart && json.cart.items) || [];
                let c = 0;
      ...
      try {
        ...
        const cardBtn = document.querySelector(
          `.button_card_add[data-id="${productId}"]`
        );
        if (cardBtn) {
          const card =
            cardBtn.closest(".product-card") ||
            cardBtn.closest(".product-item") ||
            cardBtn.parentElement;
  ...
  if (detail) {
    ...
    if (btn)
      btn.addEventListener("click", function () {
        if (showLoginModalIfNeeded()) return;
        const q = clamp(
          parseInt(qtyInput && qtyInput.value ? qtyInput.value : "1", 10)
        );
        addToCart(pid, q);
      });
  }
