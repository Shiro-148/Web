(function () {
  ...
  async function onProvinceChange() {
    ...
    try {
      const res = await fetch(
        `${wardsApiBase}/${encodeURIComponent(code)}/wards`,
        { cache: "no-store" }
      );
  ...
  function init() {
    ...
    fetch(provincesApi, { cache: "no-store" })
      .then((res) => {
        if (!res.ok)
          throw new Error("Không tải được provinces API status=" + res.status);
        return res.json();
      })
      .then((json) => {
        provinces = json;
    ...
    provinceSelect &&
      provinceSelect.addEventListener("change", updateSubmitState);
    ...
    function showConfirm(text) {
      if (typeof window.showConfirm === "function")
        return window.showConfirm(text);
      return Promise.resolve(window.confirm(text));
    }

    function showMessage(text) {
      if (typeof window.showMessage === "function")
        return window.showMessage(text);
      try {
        console.info(text);
      } catch (e) {
        /* ignore */
    ...
    function attachRowButtons() {
      document.querySelectorAll(".btn_address_delete").forEach((btn) => {
        btn.addEventListener("click", async function (e) {
          ...
          try {
            const res = await fetch(
              `${addressApiBase}/${encodeURIComponent(id)}`,
              { method: "DELETE" }
            );
            if (!res.ok) {
          ...
          try {
            ...
            if (provinceSelect) {
              ...
              for (let i = 0; i < provinceSelect.options.length; i++) {
                if (
                  provinceSelect.options[i].textContent.trim() ===
                  (addr.city || "").trim()
                ) {
            ...
            if (provinceSelect && wardSelect) {
              ...
              for (let i = 0; i < wardSelect.options.length; i++) {
                if (
                  wardSelect.options[i].textContent.trim() ===
                  (addr.ward || "").trim()
                ) {
        ...
        btn.addEventListener("click", async function () {
          ...
          try {
            if (id) {
              ...
              if (!res.ok)
                throw new Error("Load address failed: " + res.status);
              const addr = await res.json();
              if (addressInput) addressInput.value = addr.street || "";
              if (nameInput) nameInput.value = addr.receiverName || "";
              if (phoneInput) phoneInput.value = addr.phone || "";
            ...
            await showMessage(
              "Không thể mở form địa chỉ: " + (err.message || "")
            );
          }
        });
      });
    ...
    if (submitBtn) {
      submitBtn.addEventListener("click", async function (e) {
        ...
        try {
          ...
          if (editId) {
            res = await fetch(
              `${addressApiBase}/${encodeURIComponent(editId)}`,
              {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(payload),
    ...
    document.addEventListener("click", function (e) {
      if (
        e.target &&
        (e.target.id === "openAddressDialogBtn" ||
          (e.target.closest && e.target.closest("#openAddressDialogBtn")))
      ) {
